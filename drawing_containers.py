...
def draw_container(ax, index, total_width, IorE=1, W_c=1, width=20, height=8.6):
    ...
    rect = Rectangle(
        (x, y), width * W_c, height, facecolor=face, edgecolor=edge, linewidth=2
    )
    ax.add_patch(rect)
...
def draw_capacity(ax, total_height, total_width, width=20, height=8.6):
    ...
    ax.plot(
        [-margin, total_w + margin], [-margin, -margin], color=color_gray, linewidth=lw
    )

    # Right vertical line
    ax.plot(
        [total_w + margin, total_w + margin],
        [-margin, total_h],
        color=color_gray,
        linewidth=lw,
    )
